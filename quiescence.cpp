...
void insertCapture_white(int64_t list[219][5]){
    if(board.chessBoard[y_to][x_to] != empty_square){
        board.playMove_CC(0);
...
class Capture_gen{
    public:     
        void w_pawn(int64_t list[219][5]){
            if(board.chessBoard[y - 1][x] == 0){ y_to = y - 1; x_to = x; insertCapture_white(list);}
            ...
            if(y == 6){    if(board.chessBoard[y - 1][x] == 0 && board.chessBoard[y - 2][x] == 0){ y_to = y - 2; x_to = x; insertCapture_white(list);}} 
        }
        void b_pawn(int64_t list[219][5]){
            if(board.chessBoard[y + 1][x] == 0){ y_to = y + 1; x_to = x; insertCapture_black(list);}
            ...
            if(y == 1){    if(board.chessBoard[y + 1][x] == 0 && board.chessBoard[y + 2][x] == 0){ y_to = y + 2; x_to = x; insertCapture_black(list);}} 
        }
        ...
        void w_bishop(int64_t list[219][5]){
            for(int64_t i = 1; i < 8; i++){ // down & right
                ...
                if(board.chessBoard[y + i][x + i] == empty_square){ y_to = y + i; x_to = x + i; insertCapture_white(list);
        ...
        void b_bishop(int64_t list[219][5]){
            for(int64_t i = 1; i < 8; i++){ // down & right
                if(y + i > 7 || x + i > 7){ break;}
                if(board.chessBoard[y + i][x + i] == empty_square){ y_to = y + i; x_to = x + i; insertCapture_black(list);
        ...
        void w_rook(int64_t list[219][5]){
            for(int64_t i = 1; i < 8; i++){ // down
                if(y + i > 7){ break;
                } else if(board.chessBoard[y + i][x] == empty_square){ y_to = y + i; x_to = x; insertCapture_white(list);
            ...
            for(int64_t i = 1; i < 8; i++){ // left
                if(x - i < 0){ break;
                } else if(board.chessBoard[y][x - i] == empty_square){ y_to = y; x_to = x - i; insertCapture_white(list);
        ...
        void b_rook(int64_t list[219][5]){
            for(int64_t i = 1; i < 8; i++){ // down
                if(y + i > 7){ break;
                } else if(board.chessBoard[y + i][x] == empty_square){ y_to = y + i; x_to = x; insertCapture_black(list);
            ...
            for(int64_t i = 1; i < 8; i++){ // left
                if(x - i < 0){ break;
                } else if(board.chessBoard[y][x - i] == empty_square){ y_to = y; x_to = x - i; insertCapture_black(list);
        ...
        void w_king(int64_t list[219][5]){
            if(board.chessBoard[y + 1][x] == empty_square || board.chessBoard[y + 1][x] >= 11){    if(y + 1 <= 7){ y_to = y + 1; x_to = x; insertCapture_white(list);}}
            if(board.chessBoard[y - 1][x] == empty_square || board.chessBoard[y - 1][x] >= 11){    if(y - 1 >= 0){ y_to = y - 1; x_to = x; insertCapture_white(list);}}
            if(board.chessBoard[y][x + 1] == empty_square || board.chessBoard[y][x + 1] >= 11){    if(x + 1 <= 7){ y_to = y; x_to = x + 1; insertCapture_white(list);}}
            if(board.chessBoard[y][x - 1] == empty_square || board.chessBoard[y][x - 1] >= 11){    if(x - 1 >= 0){ y_to = y; x_to = x - 1; insertCapture_white(list);}}
            if(board.chessBoard[y + 1][x + 1] == empty_square || board.chessBoard[y + 1][x + 1] >= 11){    if(y + 1 <= 7 && x + 1 <= 7){ y_to = y + 1; x_to = x + 1; insertCapture_white(list);}}
            if(board.chessBoard[y + 1][x - 1] == empty_square || board.chessBoard[y + 1][x - 1] >= 11){    if(y + 1 <= 7 && x - 1 >= 0){ y_to = y + 1; x_to = x - 1; insertCapture_white(list);}}
            if(board.chessBoard[y - 1][x + 1] == empty_square || board.chessBoard[y - 1][x + 1] >= 11){    if(y - 1 >= 0 && x + 1 <= 7){ y_to = y - 1; x_to = x + 1; insertCapture_white(list);}}
            if(board.chessBoard[y - 1][x - 1] == empty_square || board.chessBoard[y - 1][x - 1] >= 11){    if(y - 1 >= 0 && x - 1 >= 0){ y_to = y - 1; x_to = x - 1; insertCapture_white(list);}}
        }
        void b_king(int64_t list[219][5]){
            if(board.chessBoard[y + 1][x] <= 6){    if(y + 1 <= 7){ y_to = y + 1; x_to = x; insertCapture_black(list);}}
            if(board.chessBoard[y - 1][x] <= 6){    if(y - 1 >= 0){ y_to = y - 1; x_to = x; insertCapture_black(list);}}
            if(board.chessBoard[y][x + 1] <= 6){    if(x + 1 <= 7){ y_to = y; x_to = x + 1; insertCapture_black(list);}}
            if(board.chessBoard[y][x - 1] <= 6){    if(x - 1 >= 0){ y_to = y; x_to = x - 1; insertCapture_black(list);}}
            if(board.chessBoard[y + 1][x + 1] <= 6){    if(y + 1 <= 7 && x + 1 <= 7){ y_to = y + 1; x_to = x + 1; insertCapture_black(list);}}
            if(board.chessBoard[y + 1][x - 1] <= 6){    if(y + 1 <= 7 && x - 1 >= 0){ y_to = y + 1; x_to = x - 1; insertCapture_black(list);}}
            if(board.chessBoard[y - 1][x + 1] <= 6){    if(y - 1 >= 0 && x + 1 <= 7){ y_to = y - 1; x_to = x + 1; insertCapture_black(list);}}
            if(board.chessBoard[y - 1][x - 1] <= 6){    if(y - 1 >= 0 && x - 1 >= 0){ y_to = y - 1; x_to = x - 1; insertCapture_black(list);}}
        }
};
